    ...
    async def process_file(file_path: Path) -> None:
        async with aiofiles.open(file_path, 'r') as f:
            try:
                ...
                print("[!] Invalid requirements.txt file. Skipping.")
    
    directory = Path(directory).resolve()
    tasks = [asyncio.create_task(process_file(file)) for file in directory.rglob('requirements.txt') if file.is_file()]
